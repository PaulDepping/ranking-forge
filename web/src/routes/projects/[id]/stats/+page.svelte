<script lang="ts">
  ...
  const trackedPlayerIds = $derived(
    new Set(data.stats.map((p) => p.player_id)),
  );
          ...
          <Card.Content class="p-3">
            <div class="mb-2 flex items-baseline justify-between">
              ...
              <span class="text-xs text-muted-foreground">
                W {player.wins.length} · L {player.losses.length} · {winRate(
                  player.wins.length,
            ...
            <div class="flex gap-2">
              <div class="flex-1">
                <p
                  class="mb-1 text-xs font-semibold uppercase tracking-wide text-green-600 dark:text-green-400"
                >
                  WINS ({player.wins.length})
                </p>
                <ScrollArea
                  class="h-24 rounded border border-border bg-muted/20"
                >
              ...
              <div class="flex-1">
                <p
                  class="mb-1 text-xs font-semibold uppercase tracking-wide text-red-600 dark:text-red-400"
                >
