...
function tournamentVisible(
  ...
  if (dateFrom && t.start_at && t.start_at.slice(0, 10) < dateFrom)
    return false;
...
function eventVisible(
  ...
  if (search.trim()) {
    ...
    if (!e.name.toLowerCase().includes(q) && !t.name.toLowerCase().includes(q))
      return false;
  }
...
function rareActiveCount(
  bracketFilter: Record<string, BracketTypeState>,
): number {
  return RARE_TYPES.filter(
...
describe("bracket type filter", () => {
  ...
  it("required type present — passes", () => {
    ...
    expect(
      eventVisible(e, t, "", null, null, "all", {
  ...
  it("excluded type present — filtered out", () => {
    ...
    const e = makeEvent({ bracket_types: ["MATCHMAKING"] });
    expect(
      eventVisible(e, t, "", null, null, "all", { MATCHMAKING: "excluded" }),
    ).toBe(false);
  });
  ...
  it("multiple required types — event must have all of them", () => {
    const t = makeTournament([]);
    const hasAll = makeEvent({
      bracket_types: ["ROUND_ROBIN", "DOUBLE_ELIMINATION"],
    });
    ...
    expect(eventVisible(missingOne, t, "", null, null, "all", filter)).toBe(
      false,
    );
  });
  ...
  it("required + excluded on different types — excluded wins when both present", () => {
    ...
    const e = makeEvent({
      bracket_types: ["DOUBLE_ELIMINATION", "MATCHMAKING"],
    });
    const filter: Record<string, BracketTypeState> = {
      DOUBLE_ELIMINATION: "required",
