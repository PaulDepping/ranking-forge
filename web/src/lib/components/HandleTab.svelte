<script lang="ts">
  ...
  let { projectId, onClose }: { projectId: string; onClose: () => void } =
    $props();
...
<div class="space-y-3">
  {#if results.length === 0}
    <div class="space-y-2">
      <Label for="handles-input"
        >One handle per line — bare handle, full slug, or full URL</Label
      >
      <Textarea
        id="handles-input"
        bind:value={input}
      ...
      {#each results as result (result.handle)}
        <div class="flex items-center gap-3 px-3 py-2 text-sm">
          {#if result.status === "created"}
            ...
            <Badge
              variant="outline"
              class="ml-auto border-green-200 bg-green-50 text-xs text-green-700"
            >
              created
            </Badge>
