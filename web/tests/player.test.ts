...
test('player detail page shows stats and tournament history', async ({ page }) => {
	await page.goto('/projects/proj-1/rankings/rank-1/players/player-1');
...
test('stats page player name links to detail page', async ({ page }) => {
	...
	await expect(page).toHaveURL(/\/projects\/proj-1\/rankings\/rank-1\/players\/player-1/);
});
...
test('back button returns to previous in-app page', async ({ page }) => {
	...
	await expect(page).toHaveURL('/projects/proj-1/rankings/rank-1/stats');
});

test('back button falls back to stats page on direct link', async ({ page }) => {
	await page.goto('/projects/proj-1/rankings/rank-1/players/player-1');
	await expect(page.getByRole('link', { name: '← Back' })).toHaveAttribute(
		'href',
		'/projects/proj-1/rankings/rank-1/stats'
	);
	await page.getByRole('link', { name: '← Back' }).click();
	await expect(page).toHaveURL('/projects/proj-1/rankings/rank-1/stats');
});
